...
var MotionSensor = {
    start: function(options) {
        ...
        if( (options.initialState && options.initialState.state) ) {
            this._motion = options.initialState.state.motion || this._motion;
        }
    },
    stop: function() {
    },
    ...
    commands: {
        emit: function(value) {
            ...
            this._motion = value;
            this._logger.debug('Emitting motion ' + value);
            dev$.publishResourceStateChange(this._resourceID, 'motion', value);
            return Promise.resolve('motion state changed to ' + value);
        },
...
module.exports = dev$.resource('Core/Devices/Virtual/MotionSensor', MotionSensor);