...
var HumiditySensor = {
    start: function(options) {
        ...
        //"reachable" event
        self.emit('reachable');
        this._humidityValue = 98;
        if( (options.initialState && options.initialState.state) ) {
            this._humidityValue = options.initialState.state.humidity || this._humidityValue;
        }
    },
    stop: function() {
    },
    ...
    commands: {
        emit: function(value) {
            ...
            this._logger.debug('Emitting humidity ' + value);
            dev$.publishResourceStateChange(this._resourceID, 'humidity', value);
            return Promise.resolve('humidity state changed to ' + value);
        },
...
module.exports = dev$.resource('Core/Devices/Virtual/HumiditySensor', HumiditySensor);