...
var Flipflop = {
    start: function(options) {
        ...
        if( (options.initialState && options.initialState.state) ) {
            this._flipflop = options.initialState.state.flipflop || this._flipflop;
        }
    },
    stop: function() {
    },
    state: {
        flipflop: {
            ...
            set: function(value) {
                this._logger.debug('Got flipflop ' + value);
    ...
    commands: {
        emit: function(value) {
            ...
            value = value || ((parseInt(Math.random() * 100) > 50) ? 'on' : 'off');
            this._flipflop = value;
            this._logger.debug('Emitting flipflop ' + value);
            dev$.publishResourceStateChange(this._resourceID, 'flipflop', value);
            return Promise.resolve('flipflop state changed to ' + value);
        },
...
module.exports = dev$.resource('Core/Devices/Virtual/Flipflop', Flipflop);