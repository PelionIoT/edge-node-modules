...
var Luminance = {
    start: function(options) {
        ...
        if( (options.initialState && options.initialState.state) ) {
            this._luminance = options.initialState.state.luminance || this._luminance;
        }
    },
    stop: function() {
    },
    ...
    commands: {
        emit: function(value) {
            ...
            this._logger.debug('Emitting luminance ' + value);
            dev$.publishResourceStateChange(this._resourceID, 'luminance', value);
            return Promise.resolve('luminance state changed to ' + value);
        },
...
module.exports = dev$.resource('Core/Devices/Virtual/Luminance', Luminance);