...
function Temperature(options){
    var self = this;
	this.clusterClass = DEFINES.CLUSTER_CLASS.MEASUREMENT_SENSING.TEMPERATURE_MEASUREMENT;
    this.znpController = options.znpController;
    this.nodeId = options.nodeId;
    this.endPoint = options.endPoint;
    this.multiplexer = options.multiplexer;
    
    self.logger = new Logger( { moduleName: 'Temperature' + this.nodeId, color: 'white'} );
    ...
    self.multiplexer.on('attrResponse ' + self.clusterClass.toString() + self.nodeId.toString(), function(info) {
        ...
        if(found) {
            ...
            self.logger.info('Got Temperature info- ' + JSON.stringify(buf));

            if(buf['status'] == 0x00 && !bypass) {
                var t = ((buf.value/100) * 9/5 + 32);
                t = Math.round(t*Math.pow(10, 2))/Math.pow(10,2);
                self.logger.info('Got temperature value- ' + t);
...
Temperature.prototype = Object.create(EventEmitter.prototype);


Temperature.prototype.setAttr = function(attrId, type, data) {
    var self = this;
    ...
    return self.multiplexer.send({workCode: DEFINES.WORK_CODE.ZCL_WRITE_ATTR, 
                            ...
                            disableDefaultRsp: false}, { commandType: 'Temperature' + attrId, overrideOld: true, priority: 'highest' }).then(function(status) {
            self.logger.debug("GOT STATUS: " + status);
    });
}

Temperature.prototype.set = function(){
...
Temperature.prototype.get = function (attrId, bypass) {
	var self = this;
    ...
    return new Promise(function(resolve, reject) {
        ...
        var commandType = 'temperature' + self.nodeId + ' ' + attrId;
        var found = false;
        for(var i = 0; i < self.requests.length; i++) {
...
Temperature.prototype.report = function (value){
	return;
};
